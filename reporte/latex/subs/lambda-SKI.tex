...
\begin{document}
    Un problema con que cuenta la compilación introducida en \ref{sssec:ski-intermedio}, llamada \textit{abstracción bracket} de Schoenfinkel, es que esta tiende
    a causar que las expresion lambda exploten cuadráticamente en tamaño al ser compiladas a SKI.\cite{lambdaToSKI} Esto lo podemos ejemplificar con la expresión $\lambda x. \lambda y. \, y \, x$, pues 
    ...
    \begin{center}
        ...
        $S (S(K \, S)\, (K \, I))(S(K \, K) \, I)$
    \end{center}
    Es por esto que en archivo \textit{Kiselyov.hs} se implementa una traducción distinta basada en la versión en \textit{OCaml} descrita en el artículo: \textit{$\lambda$ to SKI, Semantically} \cite{lambdaToSKI}.\\
    ...
    \begin{itemize}
        ...
        \[
            \infer[\textsc{EW}]
            {\Gamma+ \;\models\; d}
            {\Gamma,\sigma \;\models\; (\models K) \, \coprod\, (\Gamma+ \models d)}
        \]
        ...
        producimos un objeto sintáctico que aplica $K$ utilizando la función de aplicación $\coprod$.
        ...
        \[
            ...
            \quad
            \Gamma_2 \;\models\; d_2 }
            {\Gamma_1 \sqcup \Gamma_2  \; \models\; (\Gamma_1 \models d_1) \, \coprod \, (\Gamma_2 \models d_2)}
        \]
        Esta regla nos indica que al momento de aplicar $d_2$ a una expresión $d_1$, debemos de producir un nuevo objeto sintáctico usando la función de aplicación $\coprod$
    \end{itemize}
    Por último, tenemos las reglas para la función de aplicación $\coprod$:\cite{lambdaToSKI}
    \begin{center}
        \begin{itemize}
            \item $(\models \, d') \coprod (\models \, d) = d' \, d$
            \item $(\models \, d') \coprod (\tau_n,\ldots,\tau_1 \models \, d) = (\models \, Bd') \, (\tau_n,\ldots,\tau_1 \models \, d)$
            \item $(\tau_n,\ldots,\tau_1 \models \, d') \coprod (\models \, d) = (\models \, CCd) \, (\tau_n,\ldots,\tau_1 \models \, d')$
            \item $(\tau_n,\ldots,\tau_1 \models \, d') \coprod (\tau_m,\ldots,\tau_1 \models \, d') = (\tau_n,\ldots,\tau_1 \models (\models S) \, \coprod \, (\tau_n,\ldots,\tau_1 \models \, d')) \, \coprod \, (\tau_m,\ldots,\tau_1 \models \, d)$
        \end{itemize}
    \end{center}
\end{document}
